...
namespace flection {
cursor::cursor(const CXCursor &cursor) noexcept : _cursor(cursor) {}
...
CXCursorKind cursor::getKind() const noexcept { return (this->_cursor.kind); }

std::vector<cursor> cursor::getChildrenWithDeclKind() const noexcept {
  std::vector<cursor> ret;
  auto visitorFunc = [](CXCursor current_cursor, CXCursor parent,
    ...
    if (curKind != CXCursor_AnnotateAttr && curKind != CXCursor_FieldDecl &&
        curKind != CXCursor_CXXMethod && curKind != CXCursor_FunctionDecl&&curKind!=CXCursor_CXXBaseSpecifier) {
...
CXCursor cursor::getCursor() const noexcept { return this->_cursor; }
...
cursorType::cursorType(const CXType &type) noexcept : _type(type) {}
...
CXTypeKind cursorType::getKind() const noexcept { return this->_type.kind; }
